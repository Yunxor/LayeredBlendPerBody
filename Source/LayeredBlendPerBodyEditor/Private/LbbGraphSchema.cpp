...
#define LOCTEXT_NAMESPACE "LbbGraphSchema"

const FName ULbbGraphSchema::PC_Pose(TEXT("Pose"));
...
namespace
{
	...
	public:
		...
		virtual UEdGraphNode* PerformAction(class UEdGraph* ParentGraph, UEdGraphPin* FromPin, const FVector2f& Location, bool bSelectNewNode = true) override
		{
			...
			ULbbEdGraphNode* NewNode = NewObject<ULbbEdGraphNode>(ParentGraph, NodeClass, NAME_None, RF_Transactional);
...
void ULbbGraphSchema::GetGraphContextActions(FGraphContextMenuBuilder& ContextMenuBuilder) const
{
	const ULbbEdGraph* Graph = Cast<ULbbEdGraph>(ContextMenuBuilder.CurrentGraph);
	const ELbbGraphKind GraphKind = Graph != nullptr
		? Graph->GraphKind
...
const FPinConnectionResponse ULbbGraphSchema::CanCreateConnection(const UEdGraphPin* A, const UEdGraphPin* B) const
{
...
bool ULbbGraphSchema::TryCreateConnection(UEdGraphPin* A, UEdGraphPin* B) const
{
	return UEdGraphSchema::TryCreateConnection(A, B);
}

FLinearColor ULbbGraphSchema::GetPinTypeColor(const FEdGraphPinType& PinType) const
{
...
void ULbbGraphSchema::BreakNodeLinks(UEdGraphNode& TargetNode) const
{
	Super::BreakNodeLinks(TargetNode);
}

void ULbbGraphSchema::BreakPinLinks(UEdGraphPin& TargetPin, bool bSendsNodeNotifcation) const
{
