...
涉及文件:
- `Source/LayeredBlendPerBody/Public/LbbDefinition.h`
- `Source/LayeredBlendPerBody/Private/LbbDefinition.cpp`
- `Source/LayeredBlendPerBody/Public/LbbExecutionContext.h`
...
建议改动:
- 在 `LbbEditorModel.h` 中:
  - 删除固定输入节点数据 struct
  - 新增统一 `FLbbGraphNodeData_Input`
  - 新增 `FLbbGraphNodeData_SavePose`
  - 新增 `FLbbGraphNodeData_MakeAdditive`
  - 新增 `FLbbGraphNodeData_ApplyAdditive`
  - 删除 `FLbbGraphNodeData_ApplyMotionDelta`
- 新增一个公共图持久化结构，例如:
  - `FLbbCacheGraphModel`
- `FLbbEditorModel` 根结构增加:
  - `CacheGraph`
  - `BodyPartGraphs`
...
涉及文件:
- `Source/LayeredBlendPerBody/Public/LbbEditorModel.h`
...
建议改动:
- `LbbEdGraphNode.h/.cpp`
  - 删除固定输入节点类
  - 新增 `ULbbEdGraphNode_Input`
  - 新增 `ULbbEdGraphNode_SavePose`
  - 新增 `ULbbEdGraphNode_MakeAdditive`
  - 新增 `ULbbEdGraphNode_ApplyAdditive`
  - 删除 `ULbbEdGraphNode_ApplyMotionDelta`
- `LbbGraphNodeRegistry`
  - 新增 descriptor
...
涉及文件:
- `Source/LayeredBlendPerBodyEditor/Public/LbbEdGraphNode.h`
- `Source/LayeredBlendPerBodyEditor/Private/LbbEdGraphNode.cpp`
- `Source/LayeredBlendPerBodyEditor/Private/LbbGraphNodeRegistry.h`
- `Source/LayeredBlendPerBodyEditor/Private/LbbGraphNodeRegistry.cpp`
- `Source/LayeredBlendPerBodyEditor/Public/LbbGraphSchema.h`
- `Source/LayeredBlendPerBodyEditor/Private/LbbGraphSchema.cpp`
...
涉及文件:
- `Source/LayeredBlendPerBodyEditor/Private/LbbDefinitionEditorToolkit.h`
- `Source/LayeredBlendPerBodyEditor/Private/LbbDefinitionEditorToolkit.cpp`
...
- 公共 operator 体系已拆出单独头文件和实现文件
  ...
  - `FLbbOperatorRequirements`、`FLbbCompiledPoseSource`、`FLbbCompiledPoseTarget`、`ILbbOperatorCompileContext` 已迁到 `LbbOperators.h`
  - 运行时派生 operator 继续放在 `LbbOperators.cpp`，只有真正私有的 helper 留在匿名 namespace
- GraphNode 相关共享类型已拆分
  - Graph authoring 数据独立为 `LbbGraphNodeData.h`
  - 节点描述与编译上下文独立为 editor 私有头文件 `LbbGraphNodeTypes.h`
  - 编译消息与编译结果独立为 `LbbGraphCompiler.h`
- 运行时数据结构已独立
  - `FLbbOperatorProgramRuntimeData`
  - `FLbbRuntimeData`
  - `CacheProgram / BodyParts` 当前都由 `FLbbOperatorProgramRuntimeData` 承载
- 旧路径已清理
  - 删除了原先塞在 `LbbDefinition.cpp` 里的 compiled/operator 定义
